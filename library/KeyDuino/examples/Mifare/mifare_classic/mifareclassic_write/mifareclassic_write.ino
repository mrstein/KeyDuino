...
#define SECTOR  0 //The sector to write on (0 - 15 on Mifare 1K, 0 - 47 on Mifare 4K)
    ...
    if (uidLength == 4) {
      ...
      if (keyDuino.authenticateDefinedKey(definedKey, MIFARE_KEY_A, SECTOR) ||
        ...
        if (keyDuino.mifareclassic_IsTrailerBlock(BLOCK)) {
          ...
          keyDuino.mifareclassic_ReadDataBlock(SECTOR * 4 + BLOCK, dataRead);
          for (int i = 0; i < 16; ++i) {
        ...
        if (writeSuccess) Serial.println("Writing succeeded");
